...
void Video::setPalElem(int16 index, char red, char green, char blue,
	...
	if (_vm->getPixelFormat().bytesPerPixel == 1)
		...
		::Video::CoktelDecoder::computeHighColorMap(_vm->_global->_pPaletteDesc->highColorMap, pal, _vm->getPixelFormat(), index, 1, 0);
}
...
void Video::setPalette(PalDesc *palDesc) {
	...
	if (_vm->getPixelFormat().bytesPerPixel == 1)
		...
		::Video::CoktelDecoder::computeHighColorMap(palDesc->highColorMap, pal, _vm->getPixelFormat(), 0, numcolors);
}
...
void Video::setFullPalette(PalDesc *palDesc) {
	if (_vm->_global->_setAllPalette) {
		...
		if (_vm->getPixelFormat().bytesPerPixel == 1)
			...
			::Video::CoktelDecoder::computeHighColorMap(palDesc->highColorMap, pal, _vm->getPixelFormat());
	} else
